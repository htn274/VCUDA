...
from src.methods.DDS.masked_autoencoder import MaskedAutoencoder
from src.methods.DDS.masked_autoencoder import MaskedAutoencoderFast
...
class ProbabilisticDAGAutoencoder(nn.Module):
    def __init__(self,
                 ...
                 pd_lr=1e-3, **kwargs):  # Random seed for init. int
        super().__init__()
    ...
    def sample(self, n_samples=1):
        A = [self.probabilistic_dag.sample().unsqueeze(0) for _ in range(n_samples)]
        A = torch.concat(A) #(n_samples, N, N)
        return A.cpu().detach().numpy()

    def prob_adj(self, **kwargs):
        dag = self.probabilistic_dag.get_prob_mask()
        return dag.cpu().detach().numpy()
