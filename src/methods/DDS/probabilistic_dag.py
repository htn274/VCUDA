...
from src.methods.DDS.soft_sort import SoftSort_p1, gumbel_sinkhorn
...
class ProbabilisticDAG(nn.Module):
    ...
    def sample_edges(self):
        """
        Return a binary adjacency matrix from Gumbel Softmax
        """
    ...
    def sample_permutation(self):
        ...
        - if hard=True, return a binary matrix, otherwise return a probabilistic matrix. 
        - 2 approaches: Gumbel sinkhorn or Soft-sort topk
        """
        if self.order_type == 'sinkhorn':
    ...
    def sample(self):
        """
        Return a binary DAG adjacency matrix
        """
    ...
    def deterministic_permutation(self, hard=True):
        """
        Return a deterministic permutation by setting noise_factor=0
        """
        if self.order_type == 'sinkhorn':
            log_alpha = F.logsigmoid(self.perm_weights)
...
if __name__ == '__main__':
    ...
    dag_sampler = ProbabilisticDAG(n_nodes=10_000, hard=True, order_type='topk') 
    with Timer('Sample edge', verbose=True): 
        edges = dag_sampler.sample_edges()
    # print(f'{edges=}')
    with Timer('Sample permutation', verbose=True): 
        perm = dag_sampler.sample_permutation()
    # print(f'{perm=}')
    # adj = dag_sampler.sample()
    # print(adj)
    # perm = dag_sampler.sample_permutation()
    # print(perm)
    # print(f'{dag_sampler.edge_log_params}')
    # prob_adj = dag_sampler.get_prob_mask()
    # print(f'{prob_adj=}')
